        ...
        to enhance your stay and create unforgettable memories."/>
            ...
            <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 w-full mt-10'>
                {exclusiveOffers.map((items) =>
                    <div key={items._id} className='group relative flex flex-col
     items-start justify-between gap-1 pt-12 md:pt-18 px-4 rounded-xl
     text-white bg-no-repeat bg-cover bg-center min-h-72' style={{ backgroundImage: `url(${items.image})` }}>
                        <p className='px-3 py-1 absolute top-4 left-4 text-xs bg-white text-gray-800 font-medium rounded-full'>{items.priceOff}% OFF</p>
                        <div>
                            <p className='font-2xl font-medium font-playfair'>{items.title}</p>
